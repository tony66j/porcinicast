...
# utils.py deve già essere nel repo (è quello che usavi nella versione che funzionava)
...
OWM_KEY = os.environ.get("OPENWEATHER_API_KEY")  # facoltativo

app = FastAPI(title="PorciniCast API (v0.6)", version="0.6.1")
app.add_middleware(
    CORSMiddleware,
...
# ------------------------
# Health (per diagnostica)
# ------------------------
@app.get("/api/health")
async def health():
    return {"ok": True, "app": "Trova Porcini", "time": datetime.now(timezone.utc).isoformat()}
...
async def open_meteo_hourly(lat: float, lon: float) -> Dict[str, Any]:
    ...
    params = {
        "latitude": lat, "longitude": lon, "timezone": "auto",
        "hourly": "precipitation_probability", "forecast_days": 3
    }
    async with httpx.AsyncClient(timeout=30, headers=HEADERS) as c:
        r = await c.get(base, params=params)
        r.raise_for_status()
...
async def overpass_forest(lat: float, lon: float, radius_m:int=800) -> Optional[str]:
    ...
    try:
        ...
        return None  # più robusto: se Overpass è giù, andiamo avanti
...
# OpenWeather (opzionale): current + 5d/3h
...
def habitat_tips(aspect_oct:str, slope_deg:float, forest_label:str, elev_m:float, dry:bool)->List[str]:
    tips = []
    if dry: tips.append("Cerca versanti N–NE e conche ombreggiate per maggiore umidità.")
    else: tips.append("Controlla margini tra bosco e radure, specialmente dopo piogge.")
    if slope_deg < 2: tips.append("Evita ristagni: preferisci leggere pendenze per drenaggio.")
    elif 5 <= slope_deg <= 15: tips.append("Pendenze 5–15° spesso ideali per drenaggio e aerazione del suolo.")
    if "Fagus" in forest_label or "faggio" in forest_label.lower():
        tips.append("Nel faggio punta a zone fresche e ricche di lettiera; evita creste ventose.")
    elif "Castanea" in forest_label or "Quercus" in forest_label:
        tips.append("In castagno/quercia prova margini e viottoli ombreggiati dopo piogge regolari.")
    ...
    if band == "bassa": tips.append("A bassa quota privilegia esposizioni fresche; con caldo prolungato sali di quota.")
    elif band == "alta": tips.append("A quota alta la stagione è più tardiva: osserva finestre più avanti.")
    return tips[:4]
...
async def api_score(lat: float, lon: float, half: float = 8.0):
    ...
    tasks = [
        ...
        asyncio.create_task(open_meteo_hourly(lat, lon)),
