...
app = FastAPI(title="Trova Porcini API (Definitive)", version="2.1.1")
app.add_middleware(
    CORSMiddleware,
...
def generate_field_guide(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    other_porcini = []
    if "Latifoglie" in forest_type and 5<=month<=9: other_porcini.append({"name": "Porcino Estivo (B. aestivalis)", "desc": "Ama il caldo, cresce con querce e castagni.", "img": "https://i.imgur.com/gGkyqApp.jpg"})
    if "Latifoglie" in forest_type and 8<=month<=10: other_porcini.append({"name": "Porcino Nero (B. aereus)", "desc": "Termofilo, predilige boschi caldi e asciutti.", "img": "https://i.imgur.com/5wO5hS3p.jpg"})
    if "Conifere" in forest_type and 8<=month<=10: other_porcini.append({"name": "Porcino dei Pini (B. pinophilus)", "desc": "Tipico delle pinete montane, cappello rossiccio.", "img": "https://i.imgur.com/r3C0123p.jpg"})
    to_avoid = [{"name": "Boleto di Satana (R. satanas)", "desc": "TOSSICO. Cappello biancastro, pori rossi, viraggio intenso.", "img": "https://i.imgur.com/xT5e533p.jpg"}, {"name": "Boleto Amaro (T. felleus)", "desc": "NON COMMESTIBILE. Pori rosati, sapore amarissimo.", "img": "https://i.imgur.com/q7z5g9vp.jpg"}]
    where_to_look = []
    ...
    else: where_to_look.append(f"**Pendenza favorevole:** La pendenza moderata del sito ({slope:.0f}°) favorisce un buon drenaggio.")
...
async def api_score(lat:float=Query(...),lon:float=Query(...),half:float=Query(8.0,gt=3.0,lt=20.0)):
    ...
    for i in range(futN):
        ...
        # BUG FIX: Usare ET0_f_om (evapotraspirazione futura) invece di P_fut_om (pioggia futura)
        et7f = sum(ET0_f_om[max(0, i-6):i+1]) if ET0_f_om else 0.0
        tfit=temperature_fit(Tmin_f_blend[i],Tmax_f_blend[i],Tm_f_blend[i])
        scores.append(final_index(rolling,et7f,tfit,slope_deg,elev_m))
